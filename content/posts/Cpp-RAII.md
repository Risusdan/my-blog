...
- [目錄](#目錄)
  - [什麼是 RAII？](#什麼是-raii)
    - [RAII 的精神](#raii-的精神)
  - [為什麼需要 RAII？](#為什麼需要-raii)
    - [問題：手動管理資源容易出錯](#問題手動管理資源容易出錯)
      - [問題 1：忘記釋放資源（資源洩漏）](#問題-1忘記釋放資源資源洩漏)
      - [問題 2：提早 return 導致資源未釋放](#問題-2提早-return-導致資源未釋放)
      - [問題 3：例外導致資源未釋放](#問題-3例外導致資源未釋放)
    - [RAII 的解決方案](#raii-的解決方案)
  - [RAII 的基本實作](#raii-的基本實作)
    - [範例 1：管理動態記憶體](#範例-1管理動態記憶體)
    - [範例 2：管理檔案](#範例-2管理檔案)
    - [範例 3：管理互斥鎖](#範例-3管理互斥鎖)
  - [RAII 的例外安全保證](#raii-的例外安全保證)
    - [例外安全範例](#例外安全範例)
    - [執行結果](#執行結果)
  - [標準函式庫中的 RAII](#標準函式庫中的-raii)
    - [1. 智慧指標（Smart Pointers）](#1-智慧指標smart-pointers)
    - [2. 容器（Containers）](#2-容器containers)
    - [3. 鎖守衛（Lock Guards）](#3-鎖守衛lock-guards)
    - [4. 檔案串流（File Streams）](#4-檔案串流file-streams)
    - [5. unique\_lock（更靈活的鎖）](#5-unique_lock更靈活的鎖)
  - [自定義 RAII 類別的完整範例](#自定義-raii-類別的完整範例)
    - [範例：資料庫連線管理](#範例資料庫連線管理)
    - [執行結果](#執行結果-1)
  - [RAII 的設計原則](#raii-的設計原則)
    - [1. 一個類別管理一種資源](#1-一個類別管理一種資源)
    - [2. 建構成功即資源取得](#2-建構成功即資源取得)
    - [3. 解構子不應該拋出例外](#3-解構子不應該拋出例外)
    - [4. 考慮是否允許複製和移動](#4-考慮是否允許複製和移動)
  - [RAII vs 其他資源管理方式](#raii-vs-其他資源管理方式)
    - [1. RAII vs 手動管理](#1-raii-vs-手動管理)
    - [2. RAII vs Garbage Collection](#2-raii-vs-garbage-collection)
  - [實務建議](#實務建議)
    - [DO（應該做的）](#do應該做的)
